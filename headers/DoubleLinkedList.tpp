	...
	template <typename T>
	bool DoubleLinkedList<T>::deleteNode(T *nPointer){
		if(!_head){
			return false;
		}
		if((_head == _tail) && (&_head->data == nPointer)){
			delete _head;
			_head = nullptr;
			_tail = nullptr;
			_curNode = nullptr;
	...
	template <typename T>
	void DoubleLinkedList<T>::resetNext(){
		_curNode = _head;
	}
